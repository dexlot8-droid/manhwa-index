        ...
        ${spotlight && !searchQuery && activeGenre === 'All' ? `
            <div class="spotlight-hero">
                ...
                <div class="spotlight-content">
                    <div class="spotlight-pill">🔥 #1 Featured Manhwa</div>
                    <h1 class="spotlight-title">${escapeHtml(spotlight.title)}</h1>
                    <div class="spotlight-meta">
                        <span>📖 <strong>${spotlight.chapter_count || 40}+</strong> Chapters Available</span>
                        <span>•</span>
                        <span>Source: <strong>${escapeHtml(spotlight.source_site || 'Original')}</strong></span>
                        ${(spotlight.tags || []).slice(0, 3).map(t => `<span class="badge">${escapeHtml(t)}</span>`).join('')}
                    </div>
                    <p class="spotlight-desc">
                        Dive into the high-octane martial arts epic filled with unparalleled power growth, masteries, and cybernetic ascension in a ruthless murim world.
                    </p>
            ...
            <a href="/series/${encodeURIComponent(s.slug)}" class="series-card">
                ...
                <div class="series-card-content">
                    ...
                    <div class="series-card-meta">
                        ...
                        ${s.source_site ? `<span class="series-card-source">${escapeHtml(s.source_site)}</span>` : ''}
                    </div>
                    ${progress ? `
            ...
            <div class="hero-content">
                ...
                <div class="series-info">
                    ...
                    <div class="series-badges">
                        <span class="badge crimson">🔥 MANHWA</span>
                        <span class="badge accent">📚 ${chapters.length} Chapters</span>
                        <span class="badge">Status: ${escapeHtml(data.status || 'Ongoing')}</span>
                        ${data.source_site ? `<span class="badge source">Source: ${escapeHtml(data.source_site)}</span>` : ''}
